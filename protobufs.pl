...
In SWI-Prolog, you define your message template as a list of
predefined Prolog terms that correspond to production rules in the 
Definite Clause Grammar (DCG) that realizes the interpreter. 
Each production rule has an equivalent rule in the protobuf grammar. 
The process is not unlike specifiying the format of a
regular expression. To encode a wire-stream to variable =X=, you pass a
grounded template and a variable, =Y=, to protobuf_message/2. To decode
a wire-stream, =Y=, you pass an ungrounded template along with a
grounded wire-stream message, =Y=, to protobuf_message/2. The
...
%
protobuf_message(Protobuf, Wirestream) :-
	phrase(Protobuf, Wirestream).

protobuf_message(Protobuf, Wirestream, Residue) :-
	phrase(Protobuf, Wirestream, Residue).
